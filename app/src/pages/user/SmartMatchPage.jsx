...
import { getMatchReasons, calculateMatchScore } from '../../utils/smartMatch.js'
...
function SmartMatchPage() {
  ...
  const matches = source ? items.filter((item) => item.id !== source.id && item.type !== source.type).map((item) => ({ item, score: calculateMatchScore(source, item), reasons: getMatchReasons(source, item) })).filter((match) => match.score >= 40).sort((a, b) => b.score - a.score) : []
